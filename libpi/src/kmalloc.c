...
// track the heap start and heap end
static unsigned heap_start;
...
void *kmalloc(unsigned nbytes) {
    ...
    demand(is_aligned(heap_ptr, 4), assuming four byte aligned heap pointer);
    void *memory_block = (void *) heap_ptr;
    heap_ptr += rounded_nbytes;
    demand(heap_ptr <= heap_end, assuming there is space in the heap);
    // printk("Returning malloc'd pointer %p for %d bytes", memory_block, rounded_nbytes);
    return memory_block;
}
...
void *kmalloc_aligned(unsigned nbytes, unsigned alignment) {
    ...
    if(is_aligned(heap_ptr, alignment)) {
        return kmalloc(nbytes);
    }
...
void kmalloc_init_set_start(unsigned _addr, unsigned max_nbytes) {
    ...
    init_p = 1;
    heap_start = roundup(_addr, 4);
    heap_ptr = heap_start;
    heap_end = _addr + max_nbytes;
    // printk("Starting at %p", heap_start);
}
...
void kmalloc_init(void) {
    ...
    // printk("Setting heap start to %p", __heap_start__);
    
    kmalloc_init_set_start((unsigned) &__heap_start__, 2*bytes_per_MB*128);
}
